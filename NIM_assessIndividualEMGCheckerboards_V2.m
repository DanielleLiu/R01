...
%     set(ph(:,2:end),'YTickLabels',{},'CLim',[-1 1]*2);
    ...
    if (saveResAndFigure)
        resDir = [scriptDir '/RegressionAnalysis/RegModelResults2/'];
...
for i = 1:n_subjects 
    ...
    Data = {}; %in order: adapt, non-adapt, switch (c2ss - c1o, current naming task_switch), env (c2o - c1o, current naming, env_switch)
    %all labels should be the same, no need to save again.
    [~,~,labels,Data{1},dataRef2]=adaptDataSubject.plotCheckerboards(newLabelPrefix,ep(3,:),fh,ph(1,1),refEpTM,flip); %  EMG_split(-) - TM base, adaptation
    [~,~,~,Data{2},~] = adaptDataSubject.plotCheckerboards(newLabelPrefix,ep(4,:),fh,ph(1,2),refEpTM,flip); % EMG_split(-) - TM base, non-adapt
    
    [~,~,~,Data{3},~] = adaptDataSubject.plotCheckerboards(newLabelPrefix,ep(5,:),fh,ph(1,3),refEpOGBase,flip); % Adapt base (i.e., NIM Base) - OG base, env, current naming, env_switch)
    [~,~,~,Data{4},~] = adaptDataSubject.plotCheckerboards(newLabelPrefix,ep(7,:),fh,ph(1,4),refEpOGBase,flip); % Adapt SS - OG base, switching (current naming task_switch)
    
    [~,~,~,Data{5},~] = adaptDataSubject.plotCheckerboards(newLabelPrefix,ep(8,:),fh,ph(1,5),refEpAdaptLate,flip); %OGafter - Adaptation_{SS} , transition 1
    [~,~,~,Data{6},~] = adaptDataSubject.plotCheckerboards(newLabelPrefix,ep(11,:),fh,ph(1,6),refEpOGpost,flip); %Nimbus post early - OG post late, transition 2
...
if length(subID) > 1
    ...
    Data = {}; %in order: adapt, non-adapt, switch (c2ss - c1o, current naming task_switch), env (c2o - c1o, current naming, env_switch)
    %all labels should be the same, no need to save again.
    [~,~,labels,Data{1},dataRef2]=normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(3,:),fh,ph(1,1),refEpTM,flip); %  EMG_split(-) - TM base, adaptation
    [~,~,~,Data{2},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(4,:),fh,ph(1,2),refEpTM,flip); % EMG_split(-) - TM base, non-adapt
    
    [~,~,~,Data{3},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(5,:),fh,ph(1,3),refEpOGBase,flip); % Adapt base (i.e., NIM Base) - OG base, env, current naming, env_switch)
    [~,~,~,Data{4},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(7,:),fh,ph(1,4),refEpOGBase,flip); % Adapt SS - OG base, switching (current naming task_switch)
    
    [~,~,~,Data{5},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(8,:),fh,ph(1,5),refEpAdaptLate,flip); %OGafter - Adaptation_{SS} , transition 1
    [~,~,~,Data{6},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(11,:),fh,ph(1,6),refEpOGpost,flip); %Nimbus post early - OG post late, transition 2
...
if saveResAndFigure
    resDir = [scriptDir '/RegressionAnalysis/RegModelResults2/AllSubjectsOrGroupResults/'];
    if not(isfolder(resDir))
        mkdir(resDir)
    end
...
for normIndex = 0:1
    ...
    tableData=table(Data{1},Data{2},Data{3},Data{4},Data{5},Data{6},'VariableNames',{'Adapt', 'NonAdapt','CombinedSwitch', 'EnvSwitch', 'Trans1', 'Trans2'});
    fitTrans1NoConst=fitlm(tableData,'Trans1 ~ Adapt+NonAdapt+CombinedSwitch+EnvSwitch-1')%exclude constant
    Rsquared = fitTrans1NoConst.Rsquared

    fprintf('\n\n')

    fitTrans2NoConst=fitlm(tableData,'Trans2 ~ Adapt+NonAdapt+CombinedSwitch+EnvSwitch-1')%exclude constant
