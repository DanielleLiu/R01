...
refEpTM = defineReferenceEpoch('TMbase',ep); %with VR
...
if length(subID) > 1
    ...
    [~,~,labels,Data{1},dataRef2]=normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(3,:),fh,ph(1,1),refEpTM,flip); %  EMG_split(-) - TM base VR, adaptation
    %all labels should be the same, no need to save again.
    [~,~,~,Data{2},dataRef2]=normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(4,:),fh,ph(1,2),refEpTM,flip); %  EMG_split(+) - TM base VR, no-adapt
    [~,~,~,Data{3},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(5,:),fh,ph(1,3),refEpOGBase,flip); % TM base VR - OG base, env switching
    [~,~,~,Data{4},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(7,:),fh,ph(1,4),refEpOGBase,flip); % Adapt SS - OG base, combined switching
    ...
    [~,~,~,Data{6},~] = normalizedTMFullAbrupt.plotCheckerboards(newLabelPrefix,ep(11,:),fh,ph(1,6),refEpOGpost,flip); %TM post VR early - OG post late, transition 2
